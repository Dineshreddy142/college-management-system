...
import {
  ...
  Zap, HelpCircle, FileSpreadsheet, Sparkles, ExternalLink, Globe, UserCheck, UserX
} from "lucide-react";
...
export function ProfileModule() {
  ...
  const fetchProfile = () => {
    ...
    client.get('/face/status').then(res => {
      if (res.data?.data?.is_enrolled !== undefined) {
        setIsFaceEnrolled(res.data.data.is_enrolled);
      }
    }).catch(err => {
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
                ...
                {isFaceEnrolled === true && (
                  <span className="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 text-xs font-bold flex items-center gap-1.5 shadow-sm">
                    <UserCheck className="w-3.5 h-3.5 text-emerald-400" /> Face Registered
                  </span>
                )}
                {isFaceEnrolled === false && (
                  <span className="px-3 py-1 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/40 text-xs font-bold flex items-center gap-1.5 shadow-sm">
                    <UserX className="w-3.5 h-3.5 text-amber-400" /> Face Not Registered
                  </span>
                )}
              </div>
        ...
        <div className="relative z-10 p-6 sm:p-8 min-h-[420px]">
          ...
          {activeTab === 'security' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <ShieldCheck className="w-5 h-5 text-cyan-400" /> Security & Biometrics
                </h2>
              ...
              <div className="p-6 rounded-2xl bg-slate-800/80 border border-slate-700/80 shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-2xl border ${isFaceEnrolled ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-amber-500/10 border-amber-500/30 text-amber-400'}`}>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isFaceEnrolled === true && (
                        <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[11px] font-bold flex items-center gap-1">
                          <UserCheck className="w-3 h-3" /> Face Registered
                        </span>
                      )}
                      {isFaceEnrolled === false && (
                        <span className="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/30 text-[11px] font-bold flex items-center gap-1">
                          <UserX className="w-3 h-3" /> Face Not Registered
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-slate-400">
                      {isFaceEnrolled === true
                        ? 'Your 1:1 facial biometric profile is active and verified for secure passwordless login.'
                        : 'Face biometrics are not enrolled for your account. Register your face to enable fast login.'}
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="p-6 rounded-2xl bg-slate-800/80 border border-slate-700/80 shadow-xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-white text-base">Passkey & WebAuthn Security</h3>
                    <p className="text-xs text-slate-400 mt-1">
                      FIDO2 / WebAuthn hardware key & device biometric login support.
                    </p>
                  </div>
                </div>
