...
// Load env from .env in backend directory
...
async function setupAllDatabases() {
  ...
  // Step 1: Connect without selecting DB, drop & recreate target database
  ...
  console.log(`[1/15] Re-creating clean database '${dbName}'...`);
  ...
  try {
    ...
    // Helper runner
    const runSqlFile = async (filePath, stepLabel) => {
      console.log(`${stepLabel} Running ${path.basename(filePath)}...`);
    ...
    await runSqlFile(path.join(rootDir, 'seed.sql'), '[4/15]');

    // Step 5: Notifications table
    console.log('[5/15] Ensuring notifications table...');
    ...
    // Step 7: AI Schema
    console.log('[7/15] Creating AI Database tables...');
    await conn.query(`
    ...
    console.log('[8/15] Creating Digital Twin tables...');
    await conn.query(`
      ...
      INSERT IGNORE INTO campus_asset_categories (name, description) VALUES 
    ...
    // Step 9: Map Editor Schema
    console.log('[9/15] Creating Map Editor tables...');
    await conn.query(`
    ...
    // Step 10: Indoor Map Schema
    console.log('[10/15] Creating Indoor Map tables...');
    await conn.query(`
      CREATE TABLE IF NOT EXISTS indoor_blocks (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(100) NOT NULL,
    ...
    console.log('[11/15] Creating Navigation tables and seeding nodes/edges...');
    await conn.query(`
    ...
    // Step 12: Smart Campus Ancillary Tables
    console.log('[12/15] Creating Smart Campus ancillary tables...');
    await conn.query(`
      ...
      INSERT IGNORE INTO campus_facilities (name, icon, description) VALUES 
    ...
    console.log('[13/15] Creating Visual Editor Schema...');
    await conn.query(`
    ...
    // Step 14: System Settings & User Table Alterations
    console.log('[14/15] Configuring System Settings & User table alterations...');
    await conn.query(`
    ...
    // Step 15: Hierarchy Migration (map objects)
    console.log('[15/15] Migrating campus room objects to hierarchy...');
